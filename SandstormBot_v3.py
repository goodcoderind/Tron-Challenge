    ...
    def _get_move_impl(self, board, player_id, pos, info):
        ...
        if solo_mode and normal_moves:
            ...
            for entry in normal_moves.values():
                ...
                entry["duel_score"] = 0.6 * entry["area"] - 0.4 * enemy_area_after
                if (
                    best_normal is None
                    or entry["duel_score"] > best_normal["duel_score"]
                    or (
                        entry["duel_score"] == best_normal["duel_score"]
                        and entry["score"] > best_normal["score"]
                    )
                ):
                    best_normal = entry
        elif battle_survival_mode and normal_moves:
            filtered_moves = {}
            for move, entry in normal_moves.items():
                if entry["area"] > 20 and entry["lookahead"] > 0:
                    filtered_moves[move] = entry
            if filtered_moves:
                normal_moves = filtered_moves
                best_normal = None
                for entry in normal_moves.values():
                    if best_normal is None or entry["score"] > best_normal["score"]:
                        best_normal = entry

        if solo_mode or duel_mode or battle_survival_mode:
            boost_option = None
            phase_option = None
        else:
